...
class TransPred:
    ...
    def build_dataset(self, train_x_ar, train_y_ar, val_x_ar, val_y_ar, test_x_ar, test_y_ar):
        ...
        class TransTorchDataset(torch.utils.data.Dataset):
        ...
        self.train_dataset = TransTorchDataset(train_x_ar, train_y_ar, )
        self.val_dataset = TransTorchDataset(val_x_ar, val_y_ar, )
        self.test_dataset = TransTorchDataset(test_x_ar, test_y_ar,)
        return
    ...
    def _trans_seq_collate_fn(self, batch):

        class TransPyGDataset(pyG.data.Dataset):
    ...
    def train_model(self, batch_size=32, lr=0.001, epoch=1000, dir_cache='./'):
        ...
        train_dataloader = DataLoader(
            self.train_dataset, batch_size=batch_size, collate_fn=self._trans_seq_collate_fn, shuffle=True
        )
        val_dataloader = DataLoader(self.val_dataset, batch_size=batch_size, collate_fn=self._trans_seq_collate_fn)
    ...
    def pred_trans_test_set(self, dir_cache):
        import os
        from torch.utils.data import DataLoader
        test_dataloader = DataLoader(
            self.test_dataset, batch_size=len(self.test_dataset), collate_fn=self._trans_seq_collate_fn
        )
...
class TransPredTuner(TransPred):
